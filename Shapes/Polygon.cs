    ...
    public class Polygon : IShape {
            ...
            set
            {
                for (int row = 0; row < vertices.Count - 1; row++) {
                    for (int col = 0; col < vertices [row].Count - 1; col++) {
                        this._area = this._area + ( ( this.vertices [row] [col] * this.vertices [row + 1] [col + 1] ) - ( this.vertices [row] [col + 1] * this.vertices [row + 1] [col] ) );
                    }
                }
                this._area = _area / 2;
            }
        ...
        public double Perimeter {
            ...
            set
            {
                ...
                for (int row = 0; row < vertices.Count - 1; row++) {
                    for (int col = 0; col < vertices [row].Count - 1; col++) {
                        this._perimeter = _perimeter + Math.Sqrt(
